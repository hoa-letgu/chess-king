// src/games/chess/components/ChessBoard.tsx
import type { CSSProperties } from "react";
import { PIECE_IMG, isPieceKey, type PieceKey } from "@/games/chess/utils/pieces";
...
export function ChessBoard({
  ...
  for (let vr = 0; vr < 8; vr++) {
    for (let vc = 0; vc < 8; vc++) {
      ...
      squares.push(
        ...
        >
          {!hideStaticPiece && isPieceKey(pieceKey) && (
            <img
              src={PIECE_IMG[pieceKey]}  // ✔ OK
              style={{
                width: "80%",
                height: "80%",
                pointerEvents: "none",
                transition: "transform 0.35s ease-out",
                ...(isDeadKing ? { transform: "rotate(90deg)", opacity: 0.9 } : {}),
              }}
            />
          )}
        </button>
  ...
  // ⭐ FIX TYPE: chỉ render nếu piece hợp lệ
  if (!isPieceKey(piece)) return null;

  const step = movingPath[movingStep] || movingPath[movingPath.length - 1];

  const fileIndex = step.x;
  const rankNum = step.y;
  ...
  return (
    <img
      src={PIECE_IMG[piece]} // ✔ KHÔNG LỖI TS NỮA
      style={{
        position: "absolute",
        width: "12.5%",
        height: "12.5%",
  ...
  const renderCaptured = () => {
   ...
   let capKey: PieceKey | null = null;
  if (isPieceKey(capturedPiece.piece)) {
    capKey = capturedPiece.piece;
  }

  return capKey ? (
    <img
      src={PIECE_IMG[capKey]}
      style={{
        position: "absolute",
        width: "12.5%",
        height: "12.5%",
