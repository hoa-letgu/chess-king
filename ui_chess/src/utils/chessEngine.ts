...
function evalPieceSquare(file: string, rank: number, type: string): number {
  ...
  // Tốt tiến lên được thưởng
  if (type === "p") {
    if (rank >= 4 && rank <= 6) bonus += 4;
    if (rank === 7) bonus += 8;
  }

  // Vua an toàn hơn ở cuối
  if (type === "k") {
...
// --------------------------------------
// 5) MOVE ORDERING (SẮP XẾP NƯỚC ĐI)
// --------------------------------------
...
function orderMoves(game: Chess, moves: Move[]): Move[] {
  return moves
    .map((mv) => ({ mv, score: scoreMoveForOrdering(game, mv) }))
...
export function negamax(
  ...
  let best = -Infinity;
  const orderedMoves = orderMoves(game, game.moves({ verbose: true }) as Move[]);
